...
**)
	...
	TYPE
		...
		stat_t* = RECORD [untagged]
			...
			__pad2: SHORTINT;
			st_size*: INTEGER;	(* +64 total size, in bytes *)
		...
		Dirent* = RECORD  [untagged]
			...
			d_off*: INTEGER;	(* offset to this dirent *)
...
*)
	...
	PROCEDURE [ccall] fread* (ptr, size, nmemb: INTEGER; f: PtrFILE) : INTEGER;
	PROCEDURE [ccall] fwrite* (ptr, size, nmemb: INTEGER; f: PtrFILE) : INTEGER;
