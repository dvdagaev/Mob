...
**)
	...
	PROCEDURE (slh: StdLoaderHook) ThisMod (IN name: ARRAY OF SHORTCHAR): Kernel.Module;
		VAR mod: Kernel.Module;
	BEGIN
		mod := hookProxy.ThisMod(LONG(name));
	...
	BEGIN
		RETURN Kernel.ThisLoadedMod(SHORT(name));
	END ThisLoadedMod;
	...
	PROCEDURE EntryPoint* (IN argv: ARRAY OF INTEGER; 
	pargc, p_reg, p_body: INTEGER): INTEGER;
	BEGIN
		RunModule(0, 1, 0);
	...
	PROCEDURE SetRun* (rp: RunProc);
		VAR rc: INTEGER; argv: ARRAY 1 OF INTEGER;
	BEGIN
		IF printVars THEN
			printVars := FALSE;
			PrintVars
	...
	BEGIN
		...
		IF dir[0] = 0X THEN
			result := 1
		ELSIF Api.SetCurrentDirectoryA(dir) # 0 THEN
			result := 0
		ELSE
			result := -1
		END;
		IF Api.GetCurrentDirectoryA(LEN(odir), odir) # 0 THEN
		END
	END ChDir;
	...
	BEGIN
		...
		j := Api.DebugSetProcessKillOnExit(1);
		j := Api.GetModuleFileNameA(0, str, LEN(str));
		j := 0;
		WHILE str[j] # 0X DO 
			exeLocation[j] := LONG(str[j]);
			exePathName[j] := exeLocation[j];
			INC(j)
		END;
		exePathName[j] := 0X;
		WHILE (j > 0) & (str[j] # "\") & (str[j] # "/") & (str[j] # ":") DO DEC(j) END;
		exeLocation[j] := 0X;
		k := 0; IF j > 0 THEN INC(j) END;
		WHILE (str[j] # 0X) & (str[j] # '.') DO
			exeName[k] := LONG(str[j]);
			INC(k); INC(j)
		END;
		exeName[k] := 0X;

		j := Api.GetCurrentDirectoryA(LEN(str), str);
		j := 0;
		WHILE str[j] # 0X DO currentDir[j] := LONG(str[j]); INC(j) END;
		currentDir[j] := 0X;
		ParseArgs
	END Initialize;
