...
(** BlackBox 1.6 **)
	...
	PROCEDURE (h: LoaderHook) ThisMod* (IN name: ARRAY OF SHORTCHAR): 
	Module, NEW, ABSTRACT;
	...
	BEGIN
		loadRes := done;
		m := modList;
		WHILE (m # NIL) & ((m.name # name) OR (m.refcnt < 0)) DO
			m := m.next
		END;
	...
	BEGIN
		...
		IF size <= c.max THEN
			s := (size + (N - 1)) DIV N * N;
			adr := Api.VirtualAlloc(S.VAL(INTEGER, c), s, 
	...
	BEGIN
		IF dllMem THEN
			...
			ELSE
				...
				c.size := Api.HeapSize(hHeap, {Api.HEAP_NO_SERIALIZE}, adr) - (S.VAL(INTEGER, c) - adr);
